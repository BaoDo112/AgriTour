...
- use `/api/foo*`, not only `/api/foo/*`
- the `*` form matches both the exact base path such as `/api/bookings` and nested paths such as `/api/bookings/123`
- if you use only `/api/foo/*`, requests to the exact base path may miss the intended rule and fall through to the default action
...
- Task definition family: `agritour-tour-catalog`
- Task definition revision: use the currently validated revision in your account; in this session it is `3`
...
- Task definition family: `agritour-booking-billing`
- Task definition revision: use the currently validated revision in your account; in this session it is `4`
...
CodeDeploy note for the demo service:

- the base ECS service can start with deployment controller `ECS`
- before creating the CodeDeploy deployment group for `tour-catalog`, remove the ECS deployment circuit breaker from `agritour-tour-catalog-svc` and change that service to deployment controller `CODE_DEPLOY`
- CodeDeploy will reject the deployment group if the ECS service still uses deployment controller `ECS`
- in the current validated learner-lab session, `tour-catalog:3` is the live primary task set and was promoted by deployment `d-440L6Z25J`
- `/api/tours/featured` is the visible redeploy proof and currently returns `release = tour-catalog-codedeploy-v1`
...
2. upload the `tour-catalog` AppSpec YAML as a direct S3 object
3. note the object key and version id

Upload the AppSpec revision from your personal machine:

```powershell
Set-Location "$PROJECT_ROOT\infra\codedeploy\tour-catalog"

aws s3 cp .\appspec.yaml s3://<deployment-bucket-name>/tour-catalog/appspec.yaml --region $AWS_REGION
```

When creating the deployment, point CodeDeploy at the uploaded YAML object itself and use bundle type `YAML`.

Important learner-lab note:

- in this environment, ECS CodeDeploy rejected zip bundles with `INVALID_REVISION`
- the working path is the direct `appspec.yaml` object in S3, not a zip archive
...
1. `POST /api/auth/register` and `POST /api/auth/login` work through the ALB
2. `GET /api/tours/featured` returns `release = tour-catalog-codedeploy-v1`
3. booking creation, payment, and booking read-back finish with `bookingStatus = confirmed`
4. frontend works against the ALB URL
5. the CodeDeploy AppSpec object and rollback artifact are still available in S3

PowerShell note for ad-hoc smoke scripts:

- if you fetch `/api/tours` in PowerShell, store the parsed list in a variable before reading `.tour_id`
- avoid sending the raw `.tour_id` property column directly into `/api/bookings`, because PowerShell can serialize it as an array instead of a single tour ID
